"""Baseline: always generate direct answers (no CoT) for held-out questions.

Runs on Modal T4 GPU. Evaluates accuracy and token usage when the model
always answers directly without chain-of-thought reasoning.

Usage:
    modal run scripts/evaluate_baseline_direct.py --num-questions 200

Cost estimate:
    200 questions x ~5 s generation each ≈ 0.3 hours on T4
    Estimated total: $0.15-$0.20
"""
...
app = App("heldout-baseline-direct")
...
def evaluate_baseline_direct(num_questions: int = 200) -> dict:
    """Run baseline evaluation: always direct answers."""
    import numpy as np
    import torch
    ...
    for q in tqdm(heldout, desc="Direct"):
        prompt = q["prompt"]
        dataset = q["dataset"]
    ...
    output_path = output_dir / "baseline_direct_results.json"
    with open(output_path, "w") as f:
        json.dump(summary, f, indent=2, default=float)

    print("\n" + "=" * 60)
    print("BASELINE DIRECT COMPLETE")
    print("=" * 60)
...
def main(num_questions: int = 200):
    evaluate_baseline_direct.remote(num_questions)
...
if __name__ == "__main__":
    ...
    print(
        ...
        f"  modal run scripts/evaluate_baseline_direct.py --num-questions {args.num_questions}"
    )
