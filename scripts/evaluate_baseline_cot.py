"""Baseline: always generate CoT answers for held-out questions.

Runs on Modal T4 GPU. Evaluates accuracy and token usage when the model
always uses chain-of-thought reasoning before answering.

Usage:
    modal run scripts/evaluate_baseline_cot.py --num-questions 200

Cost estimate:
    200 questions x ~20 s generation each ≈ 1.1 hours on T4
    Estimated total: $0.60-$0.70
"""
...
def evaluate_baseline_cot(num_questions: int = 200) -> dict:
    """Run baseline evaluation: always CoT answers."""
    import numpy as np
    import torch
    ...
    print("\n── Generating CoT answers ──")
    results = []
    total_tokens = 0

    for q in tqdm(heldout, desc="CoT"):
        prompt = q["prompt"]
        dataset = q["dataset"]
    ...
    output_path = output_dir / "baseline_cot_results.json"
    with open(output_path, "w") as f:
        json.dump(summary, f, indent=2, default=float)

    print("\n" + "=" * 60)
    print("BASELINE CoT COMPLETE")
    print("=" * 60)
...
def main(num_questions: int = 200):
    evaluate_baseline_cot.remote(num_questions)
...
if __name__ == "__main__":
    ...
    print(
        ...
        f"  modal run scripts/evaluate_baseline_cot.py --num-questions {args.num_questions}"
    )
