"""Full gen-time probe training: ALL 197 questions + calibration analysis."""
...
def train_probes_full():
    ...
    if os.path.exists(all_pkl_path):
        ...
        print(f"Loaded {len(all_results)} entries from all_results.pkl")

    n_loaded_from_files = 0
    if os.path.isdir(QUESTIONS_DIR):
        qfiles = sorted([f for f in os.listdir(QUESTIONS_DIR) if f.endswith('.pkl')])
        print(f"Found {len(qfiles)} individual question files")
        for qf in tqdm(qfiles, desc="Loading individual files"):
            qid = qf.replace('.pkl', '')
            if qid in all_results and isinstance(all_results[qid], dict) and 'hidden_states' in all_results[qid] and len(all_results[qid].get('hidden_states', [])) > 0:
                continue
            try:
                with open(f"{QUESTIONS_DIR}/{qf}", 'rb') as f:
                    data = pickle.load(f)
                if isinstance(data, dict) and 'hidden_states' in data and len(data.get('hidden_states', [])) > 0:
                    all_results[qid] = data
                    n_loaded_from_files += 1
            except Exception as e:
                print(f"  WARN: {qf}: {e}")
        print(f"Loaded {n_loaded_from_files} entries from individual files")
    ...
    print(f"Questions with hidden states: {n_with_hs}")
    ...
    if aurocs:
        first_q = [p for p in position_results if p["token_index"] <= max(indices) * 0.25]
        last_q = [p for p in position_results if p["token_index"] >= max(indices) * 0.75]
        first_quarter = np.mean([p["test_auroc"] for p in first_q]) if first_q else 0.0
        last_quarter = np.mean([p["test_auroc"] for p in last_q]) if last_q else 0.0
    ...
    output = {
        ...
        "calibration_summary": {
            "n_bins": 10,
            "note": "ECE computed with 10 equal-width bins. < 0.05 = well-calibrated, > 0.10 = needs fixing",
        },
        "avg_metrics": {
            ...
            "ece_at_peak": round(best_pos.get("ece", 0.0), 4) if best_pos else 0.0,
            "brier_at_peak": round(best_pos.get("brier_score", 0.0), 4) if best_pos else 0.0,
        },
    }
    ...
    print(f"  Optimal position: {best_pos.get('token_index', 'N/A')}")
    print(f"  Test AUROC:       {best_pos.get('test_auroc', 0):.4f}")
    print(f"  Train AUROC:      {best_pos.get('train_auroc', 0):.4f}")
    print(f"  Overfitting gap:  {best_pos.get('overfitting_gap', 0):.4f}")
    print(f"  Brier score:      {best_pos.get('brier_score', 0):.4f}")
    print(f"  ECE (10-bin):     {best_pos.get('ece', 0):.4f}")
    print(f"  Scenario:         {scenario}")
    print(f"  Avg test AUROC:   {output['avg_metrics']['test_auroc_mean']:.4f}")
    print(f"  Avg overfit gap:  {output['avg_metrics']['overfitting_gap_mean']:.4f}")

    if best_pos.get('ece', 1.0) < 0.05:
        print(f"\n  CALIBRATION: WELL-CALIBRATED (ECE < 0.05)")
    elif best_pos.get('ece', 1.0) < 0.10:
        print(f"\n  CALIBRATION: ACCEPTABLE (ECE < 0.10)")
    else:
        print(f"\n  CALIBRATION: NEEDS FIXING (ECE >= 0.10)")
...
def main():
    result = train_probes_full.remote()
    opt = result.get('optimal_position', {})
    print(f"\nDone. Pos={opt.get('token_index')}, AUROC={opt.get('test_auroc')}, ECE={opt.get('ece')}")
